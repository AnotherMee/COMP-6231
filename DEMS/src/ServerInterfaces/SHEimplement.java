...
public class SHEimplement extends UnicastRemoteObject implements Interfaces
{
	public SHEimplement she;
	public BookingCapacity sheCapAndList;
	public HashMap<String, HashMap<String , BookingCapacity>> sheEventRecords = new HashMap<>();
	public HashMap<String, List<String>> she_customerHM = new HashMap<>();
	
	public SHEimplement() throws RemoteException 
	{
		super();
	}
	...
	public String addEvent(String eventID, String eventType, BookingCapacity bookCap) throws RemoteException 
	{
		...
		if (sheEventRecords.containsKey(eventType))
		{
			HashMap<String, BookingCapacity> sheEvents = sheEventRecords.get(eventType);
			
			if (sheEvents.containsKey(eventID))
			{
				
				sheEvents.put(eventID,bookCap);
				sheEventRecords.put(eventType, sheEvents);
		...
		else
		{
			...
			HashMap<String, BookingCapacity> sheEvents = new HashMap<>();
			sheEvents.put(eventID,bookCap);
			sheEventRecords.put(eventType, sheEvents);
			return "Add an event successful";
		}
	...
	public synchronized String removeEvent(String eventID, String eventType) throws RemoteException
	{
		...
		if (sheEventRecords.containsKey(eventType))
		{
			...
			if (sheEvents.containsKey(eventID))
			{
				sheCapAndList = sheEvents.get(eventID);
				List<String> idlist = sheCapAndList.getCustomerID();
				List<String> list = new LinkedList(sheEvents.keySet());
				...
				{
					sheCapAndList = sheEvents.get(list.get(j));
					int cap = sheCapAndList.getBookcap();
	...
	public synchronized String listEventAvailability(String eventType)throws RemoteException
	{
		...
		{
			HashMap<String, BookingCapacity> sheEvents = sheEventRecords.get(eventType);
			for (String key: sheEvents.keySet())
			{
				eventIDs = key;
				caps = String.valueOf(sheEvents.get(key).getBookcap());
	...
	public synchronized String bookEvent(String customerID, String eventID, String eventType) throws RemoteException 
	{
		...
		{			
			if (sheEventRecords.containsKey(eventType))
			{			
				HashMap<String, BookingCapacity> mtlEvents = sheEventRecords.get(eventType);
				
				if (mtlEvents.containsKey(eventID))
				{					
					sheCapAndList = mtlEvents.get(eventID);
					if (sheCapAndList.getBookcap() != 0)
					{
						
						sheCapAndList.addCustomerID(customerID);
						sheCapAndList.setBookcap(sheCapAndList.getBookcap());
						if (she_customerHM.containsKey(customerID))
						{
							List <String> eventIDs = new ArrayList();
							eventIDs = she_customerHM.get(customerID);
							eventIDs.add(eventID);
							she_customerHM.put(customerID, eventIDs);							
						}
						else
						{
							List <String> eventIDs = new ArrayList();
	...
	public synchronized String cancelEvent(String customerID, String eventID, String eventType) throws RemoteException 
	{
		...
		if(server.equals("SHE"))
		{
			if (sheEventRecords.containsKey(eventType))
			{
				HashMap<String, BookingCapacity> events = sheEventRecords.get(eventType);
